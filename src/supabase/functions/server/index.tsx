...
app.get("/make-server-8a20b27d/reports/entries", async (c) => {
  try {
    ...
    const formattedEntries = (entries || []).map((entry: any) => ({
      ...
      unitPrice: parseFloat(String(entry.unit_price ?? entry.unit_cost ?? 0)),
      totalPrice: parseFloat(String(entry.total_price ?? entry.total_cost ?? 0)),
      entryDate: entry.entry_date,
      batchNumber: entry.batch_number,
      expirationDate: entry.expiration_date ?? entry.expiry_date,
      notes: entry.notes,
